...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'agentmesh-uninstall-'));
...
const flagPath = path.join(claudeDir, '.agentmesh-mode');
fs.writeFileSync(flagPath, 'full');

const configDir = path.join(temp, 'config-home', 'agentmesh');
fs.mkdirSync(configDir, { recursive: true });
const configPath = path.join(configDir, 'config.json');
fs.writeFileSync(configPath, JSON.stringify({ defaultMode: 'ultra' }));

const settingsPath = path.join(claudeDir, 'settings.json');
const installedStatuslinePath = path.join(claudeDir, 'agentmesh-statusline.sh');
fs.writeFileSync(installedStatuslinePath, '#!/usr/bin/env bash\n');
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash /some/path/agentmesh-statusline.sh' },
}));
...
assert.equal(fs.existsSync(installedStatuslinePath), false, 'installed statusline script must be removed');

const settingsAfter = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
assert.equal(
  settingsAfter.statusLine,
  undefined,
  'Agentmesh statusLine entry must be removed',
);
...
// A combined statusline (another plugin && Agentmesh) must keep the other
// plugin's part — uninstall must not nuke the whole command or leave a husk.
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash ~/caveman-statusline.sh && bash /p/mesh-statusline.sh' },
}));
...
assert.equal(
  ...
  'a combined statusLine must keep the non-Agentmesh command',
);

// A separately installed Ponytail plugin owns its own statusline. Agentmesh
// uninstall must leave it untouched.
fs.writeFileSync(settingsPath, JSON.stringify({
...
assert.equal(result.status, 0, result.stderr);
assert.equal(
  JSON.parse(fs.readFileSync(settingsPath, 'utf8')).statusLine.command,
  'bash /p/ponytail-statusline.sh',
  'uninstall must not alter a different plugin statusLine',
);
...
const malformedSettings = '{ "statusLine": { "command": "agentmesh-statusline.sh", broken';
