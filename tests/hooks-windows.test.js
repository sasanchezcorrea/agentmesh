...
test('mesh-mode-tracker self-exits when stdin never closes (no freeze)', async () => {
  const hook = path.join(root, 'hooks', 'mesh-mode-tracker.js');
...
test('PowerShell statusline uses Agentmesh state and branding', () => {
  const script = fs.readFileSync(path.join(root, 'hooks', 'mesh-statusline.ps1'), 'utf8');
  assert.match(script, /\.agentmesh-mode/);
  assert.match(script, /\[MESH/);
  assert.doesNotMatch(script, /PONYTAIL/i);
});

test('Claude plugin manifest points at the shared host-specific hook config', () => {
