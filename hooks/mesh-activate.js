#!/usr/bin/env node
// agentmesh — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes mode state at $CLAUDE_CONFIG_DIR/.agentmesh-mode (defaults to ~/.claude)
//   2. Emits agentmesh rules as hidden SessionStart context
//   3. Detects missing statusline config and emits setup nudge

const fs = require('fs');
const path = require('path');
const { getDefaultMode, getClaudeDir, isShellSafe } = require('./mesh-config');
const { getMeshInstructions } = require('./mesh-instructions');
const {
  clearMode,
  isCopilot,
  setMode,
...
// 2. Emit the agentmesh rules, filtered to the active intensity level.
let output = getMeshInstructions(mode);
...
if (!isCopilot) try {
  ...
  const nudgeFlagPath = path.join(claudeDir, '.agentmesh-statusline-nudged');
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    try { fs.writeFileSync(nudgeFlagPath, ''); } catch (e) { /* best-effort */ }
    const isWindows = process.platform === 'win32';
    const scriptName = isWindows ? 'mesh-statusline.ps1' : 'mesh-statusline.sh';
    ...
    if (isShellSafe(scriptPath)) {
      ...
      output += "\n\n" +
        "STATUSLINE SETUP NEEDED: agentmesh includes a statusline badge showing active mode " +
        "(e.g. [MESH], [MESH:ULTRA]). It is not configured yet. " +
        "To enable, add this to " + settingsPath + ": " +
        statusLineSnippet + " " +
        "Proactively offer to set this up for the user on first interaction.";
    } else {
      // mesh: install path has shell metacharacters — don't embed it in a
