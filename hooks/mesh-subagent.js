#!/usr/bin/env node
// agentmesh — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent misses the active Agentmesh mode.
// When mesh mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in): set AGENTMESH_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./mesh-runtime');

const mode = readMode();

// Absent flag or off → Agentmesh isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getMeshInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.AGENTMESH_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.AGENTMESH_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
